...
/// load the sprites
class Sprites {
  final Uint8List _result = Uint8List(256 * 240 * 4);

  /// number of sprites in a scanline
  final List<int> _nb_sprites = List.filled(240, 0);

  /// priority of a pixel
  final List<bool> _has_priority = List.filled(256 * 240, false);

  /// sprite 0 opaque pixels
  final List<bool> _sprite0_opaque_pixels = List.filled(256 * 240, false);
  ...
  void _render() {
    ...
    int pattern_address = ppu.pattern_sprites_location * 0x1000;
    final bool is8x16 = ppu.has8x16Sprites;

    for (int id = 0; id < 64; id++) {
      int addr = id * 4;
      int start_y = ppu.memory._spr_ram[addr] + 1;
      int start_x = ppu.memory._spr_ram[addr + 3];
      int pattern_index = ppu.memory._spr_ram[addr + 1];
      int attributes = ppu.memory._spr_ram[addr + 2];
      ...
      int pattern_start =
          is8x16 ? 0x1000 * (pattern_index & 1) | ((pattern_index >> 1) << 5) : pattern_address | (pattern_index << 4);
      ...
      for (int y = start_y; y < start_y + (is8x16 ? 16 : 8); y++) {
        ...
        int y_pattern_pos = swap_verti ? ((is8x16 ? 15 : 7) - (y - start_y)) : (y - start_y);
        ...
        for (int x = start_x; x < start_x + 8; x++) {
          ...
          int color = high_palette |
              ((ppu.memory._data[pattern_start + y_pattern_pos] >> (7 - x_pattern_pos)) & 1) |
              (((ppu.memory._data[pattern_start + 8 + y_pattern_pos] >> (7 - x_pattern_pos)) & 1) << 1);
