...
class Background {
  ...
  static const List<List<int>> _mirroring_tables = [
    // Horizontal
    [0, 0, 0x400, 0x400],
    // Vertical
    [0, 0x400, 0, 0x400],
  ...
  void _render_tile_line(int start_x, int start_y) {
    int pattern_loc = ppu.pattern_background_location * 0x1000;
    ...
    for (int old_col = cols_start; old_col < cols_end; old_col++) {
      ...
      table_offset = _mirroring_tables[ppu.memory._mirroring.index][mirroring_offset];
      int real_tile = line * 32 + col;
      // get the high bit
      int square = ((col & 2) >> 1) + (line & 2);
      int number = (col >> 2) + ((line >> 2) << 3);
      int addr = 0x23C0 + table_offset + number;
      int high_bit = ((ppu.memory._data[addr] >> (2 * square)) & 3) << 2;

      int pattern = (ppu.memory._data[0x2000 + table_offset + real_tile] << 4) + pattern_loc;

      // Now we can render the pixels
      for (int x = 0; x < 8; x++) {
        for (int y = 0; y < 8; y++) {
          int color = high_bit |
              ((ppu.memory._data[pattern + y] >> (7 - x)) & 1) |
              (((ppu.memory._data[pattern + 8 + y] >> (7 - x)) & 1) << 1);
          _result[(old_line * 8 + y) * 256 * 2 + ((old_col & 0x3F) * 8 + x)] = color;
        }
      }
    }
  }
